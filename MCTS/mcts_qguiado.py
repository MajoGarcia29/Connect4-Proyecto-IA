...
class MCTSQGuiadoAgent(Policy):
    ...
    def __init__(
        ...
        ruta_pkl = os.path.join(current_dir, "qvalues_majo.pkl")
        
        try:
            with open(ruta_pkl, "rb") as f:
                data = pickle.load(f)
                self._q_table = data.get("q", data) if isinstance(data, dict) else data
        except FileNotFoundError:
            print(f"[MCTSQGuiado] ADVERTENCIA: no se encontró '{ruta_pkl}'.")
            self._q_table = {}
