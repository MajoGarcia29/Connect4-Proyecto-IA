...
from groups.tbopiv3.policy import TBOPIPolicy
...
class TrainConfig:
    ...
    qtable_path: str       = "groups/tbopiv3/qtable.pkl"
...
def snapshot_qtable(qtable_path: Path, snapshots_dir: Path, round_n: int) -> Path:
    ...
    dest = snapshots_dir / f"qtable_round_{round_n:04d}.pkl"
...
def train(config: TrainConfig) -> None:
    ...
    if qtable_path.exists():
        print(f"[*] Cargando Q-table existente desde: {qtable_path}")
    else:
        print(f"[!] No se encontró Q-table en {qtable_path}. Se creará una nueva.")
    ...
    for round_n in range(1, config.rounds + 1):
        ...
        avg_time_per_round = elapsed / round_n
        remaining_rounds = config.rounds - round_n
        eta_seconds = int(avg_time_per_round * remaining_rounds)
        eta_str = str(timedelta(seconds=eta_seconds))

        print(f"Ronda {round_n:3d}/{config.rounds}  |  {stats}  "
              f"| pool={len(pool)} | ETA: {eta_str}")

        # Guardar snapshot periódicamente y agregarlo al pool
        if round_n % config.snapshot_every == 0:
            # Persistir el aprendizaje actual en el archivo antes de copiarlo
            if hasattr(agent, 'save_qtable'):
                agent.save_qtable()
    ...
    # Guardar Q-table final al terminar el entrenamiento
    if hasattr(agent, 'save_qtable'):
        agent.save_qtable()
    ...
    if hasattr(agent, '_qtable'):
        print(f"Q-table final: {qtable_path} ({len(agent._qtable)} entradas)")
    else:
        print(f"Q-table final guardada en: {qtable_path}")
...
def parse_args() -> TrainConfig:
    ...
    parser.add_argument("--qtable_path",       type=str,   default="groups/tbopiv3/qtable.pkl")
